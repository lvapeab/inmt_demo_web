...
if (flock($fp, LOCK_EX))
{
	...
	$url = '127.0.0.1:6542/?vid_idx='.urlencode($vid_idx).'&prefix='.urlencode($prefix).'&learn='.urlencode($learn).'&beam_size='.urlencode($beam_size).'&length_norm='.urlencode($length_norm).'&coverage_norm='.urlencode($coverage_norm).'&alpha_norm='.urlencode($alpha_norm);
    $ch = curl_init();
